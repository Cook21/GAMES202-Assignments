...
Vec3f IntegrateEmu(float roughness, float mu, Vec3f Ei)
{
    return Ei * 2.0 * mu;
}
...
Vec3f getEmu(int x, int y, int alpha, unsigned char* data)
{
    ...
    if (Edata == NULL) {
        ...
        for (int i = 0; i < resolution; i++) {
            ...
            for (int j = 0; j < resolution; j++) {
                ...
                float sinTheta = std::sqrt(1.f - NdotV * NdotV);
                float dCosTheta = step;
                Vec3f V = Vec3f(sinTheta, 0.f, NdotV);
                // 因为之前flip过，所以是resolution - 1 - i
                Vec3f Ei = getEmu((resolution - 1 - i), j, 0, Edata);
                Eavg += IntegrateEmu(roughness, NdotV, Ei) * dCosTheta;
                //setRGB(i, j, Ei, data);
            }
            //每行都是一样的值
            ...
            Eavg = Vec3f(0.0); //Reset on each loop
        }
