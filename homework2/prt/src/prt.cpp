                                                    ...
                                                    const int &channel)
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                {
                    float u = 2.0f * ((x + 0.5) / width) - 1;
                    float v = 2.0f * ((y + 0.5) / height) - 1;
                    //所有向量的末端都在z=1平面上，代表cubemap的一个面
        ...
        for (int texIndex = 0; texIndex < 6; texIndex++)
        {
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    Eigen::Vector3f dir = cubemapDirs[texIndex * width * height + y * width + x];
                    int index = (y * width + x) * channel;
                    Eigen::Array3f Le(images[texIndex][index + 0], images[texIndex][index + 1],
                                      images[texIndex][index + 2]);
                    int i=0;
                    for(int l=0;l<=SHOrder ;l++){
                        for(int m=-l;m<=l;m++){
                            SHCoeffiecents[i] += Le * sh::EvalSH(l,m,dir.cast<double>()) * CalcArea(x, y, width, height);
                            i++;
                        }
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        for (int i = 0; i < mesh->getVertexCount(); i++)
        {
            ...
            auto shFunc = [&](double phi, double theta) -> double { 
                const Vector3f wi = sh::ToVector(phi, theta).cast<float>(); //返回值是单位向量
                ...
                else
                {
                    ...
                    if(n.dot(wi)>0 && !scene->rayIntersect(Ray3f(v,wi))){
                        return n.dot(wi);
                    }else{
                        return 0;
                    }
